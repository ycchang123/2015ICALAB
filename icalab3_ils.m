function  data=icalab3_ils(src_dir,algo,filter_order,cut_freq)
fs=56000;
...
for i=1:length(data)
    ...
    filter_Y32(1,:)=filter(b,a,Y32(1,:));filter_Y32(2,:)=filter(b,a,Y32(2,:));
    [filter_ECAPc filter_ECAPa]=icalab3_pinv(S32,filter_Y32);
    filter_ecapICA32=0.5*(filter_ECAPc+filter_ECAPa);
    data(i).ecap32=ecap32;data(i).filter_ecapICA32=filter_ecapICA32;
    ...
    filter_Y64(1,:)=filter(b,a,Y64(1,:));filter_Y64(2,:)=filter(b,a,Y64(2,:));
    [filter_ECAPc filter_ECAPa]=icalab3_pinv(S64,filter_Y64);
    filter_ecapICA64=0.5*(filter_ECAPc+filter_ECAPa);
